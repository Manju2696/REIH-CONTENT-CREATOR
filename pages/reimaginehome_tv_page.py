...
def show():
    ...
    if st.session_state.tv_fullscreen_video:
        ...
        # Back button
        if st.button("← Back to Grid", key="tv_back_button"):
            st.session_state.tv_fullscreen_video = None
            st.rerun()
        
        st.markdown("### 🎬 Now Playing")
        
        # Create scrollable player with autoplay
        player_html = f"""
        <!DOCTYPE html>
        <html>
        <head>
            ...
            for j, col in enumerate(cols):
                if i + j < len(videos):
                    ...
                    with col:
                        ...
                        # Play button
                        if st.button("▶️ Play", key=f"play_{video_db_id}", use_container_width=True):
